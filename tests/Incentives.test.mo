...
suite("Reward", func(){
    ...
    test("Compute score", func(){
        ...
        var score = Incentives.compute_score({ choice = #YES; total_yes = 500; total_no = 500; });
        verify(score, 0.5, Testify.float.equalEpsilon9);
        score := Incentives.compute_score({ choice = #NO; total_yes = 500; total_no = 500; });
        verify(score, 0.5, Testify.float.equalEpsilon9);
        
        // 2. The total number of yes is 3 times the total number of no
        
        // Verify the score is greater than 0.75 when voting YES
        score := Incentives.compute_score({ choice = #YES; total_yes = 300; total_no = 100; });
        verify(score, 0.75, Testify.float.greaterThan);
        
        // Verify the score is less than 0.25 when voting NO
        score := Incentives.compute_score({ choice = #NO; total_yes = 300; total_no = 100; });
    ...
    test("Compute contest", func(){
        ...
        var contest = Incentives.compute_contest({ choice = #YES; amount = 1; total_yes; total_no; });
        verify(contest, 0.333, Testify.float.equalEpsilon3);

        // For 200 coins, the contest per coin shall be less 0.333, but more than 0.2
        contest := Incentives.compute_contest({ choice = #YES; amount = 200; total_yes; total_no; });
        ...
        contest := Incentives.compute_contest({ choice = #YES; amount = 1; total_yes = total_yes + 199; total_no; });
        ...
        contest := Incentives.compute_contest({ choice = #NO; amount = 1; total_yes; total_no; });
        ...
        contest := Incentives.compute_contest({ choice = #NO; amount = 100; total_yes; total_no; });
        ...
        contest := Incentives.compute_contest({ choice = #NO; amount = 1; total_yes; total_no = total_no + 99; });
