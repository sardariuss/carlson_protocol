...
suite("Reward", func(){
    ...
    test("Compute score", func(){
        ...
        var score = Reward.compute_score({ choice = #YES(1); total_yes = 500; total_no = 500; });
        verify(score, 0.5, Testify.float.equalEpsilon9);
        score := Reward.compute_score({ choice = #NO(1); total_yes = 500; total_no = 500; });
        verify(score, 0.5, Testify.float.equalEpsilon9);
        
        // 2. The total number of yes is 3 times the total number of no
        
        // Verify the score is greater than 0.75 when voting AYE
        score := Reward.compute_score({ choice = #YES(1); total_yes = 300; total_no = 100; });
        verify(score, 0.75, Testify.float.greaterThan);
        
        // Verify the score is less than 0.25 when voting NAY
        score := Reward.compute_score({ choice = #NO(1); total_yes = 300; total_no = 100; });
    ...
    test("Compute contest", func(){
        ...
        var contest = Reward.compute_contest({ choice = #YES(1); total_yes; total_no; });
        verify(contest, 0.333, Testify.float.equalEpsilon3);

        // For 200 coins, the contest per coin shall be less 0.333, but more than 0.2
        contest := Reward.compute_contest({ choice = #YES(200); total_yes; total_no; });
        ...
        contest := Reward.compute_contest({ choice = #YES(1); total_yes = total_yes + 199; total_no; });
        ...
        contest := Reward.compute_contest({ choice = #NO(1); total_yes; total_no; });
        ...
        contest := Reward.compute_contest({ choice = #NO(100); total_yes; total_no; });
        ...
        contest := Reward.compute_contest({ choice = #NO(1); total_yes; total_no = total_no + 99; });
