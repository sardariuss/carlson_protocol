...
await Async.suite("GodwinProtocol", func(): async () {

    await Async.test("Actor", func(): async() {
        let protocol = await GodwinProtocol.GodwinProtocol({
            deposit_ledger = Principal.fromText("aaaaa-aa");
            reward_ledger = Principal.fromText("aaaaa-aa");
            lock_parameters = {
                nominal_duration_per_sat = #MINUTES(5);
...
suite("Decay", func(){
    ...
    func unwrap_lock(lock: ?Protocol.TokensLock) : Protocol.TokensLock {
        switch(lock) {
            case (?l) { l; };
