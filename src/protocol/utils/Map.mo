...
module {
    ...
    public let acchash: HashUtils<(Principal, ?Blob)> = (
        func(key) =
            switch (key.1) {
                case (null) { Prim.hashBlob(Prim.blobOfPrincipal(key.0)) };
                case (?blob) { ((Prim.hashBlob(Prim.blobOfPrincipal(key.0)) << 1) ^ Prim.hashBlob(blob)) & 0x3fffffff };
            },
        func(a, b) = a.0 == b.0 and a.1 == b.1,
    );
